	...
	$.fn.extend({
		triggerInputMac : function(arg){
			...
			function format(string){
				...
				// Ã¦Å“â‚¬Ã¥ÂÅ½Ã¦Â·Â»Ã¥Å   splitor
				if(formatedMac.length < 15 && (formatedMac.length-2)%3 === 0){
					formatedMac+= splitor;
				}
				if(upperCase == null){
					// Ã¤Â¸ÂÃ¨â€¡ÂªÃ¥Å Â¨Ã¨Â½Â¬Ã¦ÂÂ¢Ã¥Â¤Â§Ã¥Â°ÂÃ¥â€ â„¢
					return formatedMac;
				}
...
alerta('loader','Preparando impresiÃ³n...');
$('#printframe').remove();
var t = $("<iframe></iframe>");
...
getmodal2('ajax/viewuser?action=mapauser&idservicio='+$(this).data('mapa'),'UbicaciÃ³n Cliente','lg');
});
...
$.post( "ajax/usuarios", { action: "resendsms", id: id })
  .done(function( data ) {
 alerta('exito','El sms esta en cola para su envÃ­o.');
...
$('.counter').html('CarÃ¡cteres <b>'+$(caja).val().length+'/'+limite+'</b>');
}

function send_contrato(id,tipo){
var mjss;
if(tipo>0){
mjss='Esta seguro que desea enviar por correo el contrato <b>NÃ‚Âº '+id+'</b>';
}else{
...
$.post( "ajax/usuarios", { action: "resendmail", id: id })
  .done(function( data ) {
 alerta('exito','El correo esta en cola para su envÃ­o.');
$('a[href="#nav-tab-5"]').trigger('click');
...
alerta('warning','No es posible Obtener su ubicaciÃ³n actual ('+err.message+')');
$('.btn-ubi').html('<i class="fas fa-location-arrow"></i>');
$('[data-toggle="tooltip"]').tooltip('hide');
}
...
$('#loginForm #login').attr('placeholder', 'NÃºmero de telÃ©fono');
